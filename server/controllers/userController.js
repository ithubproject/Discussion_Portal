...
const getUser = asyncHandler(async (req, res) => {
  ...
  const { _id, name, email, DOB, level, membership, image, isAdmin } = user;
  res.status(200).json({
    _id,
    name,
    email,
...
const updateUser = asyncHandler(async (req, res) => {
  ...
  if (req.file) {
    ...
    const deleted = await cloudinary.uploader.destroy(user.image.fileId);
  ...
  user.image = imageData.imagePath ? fileData : user.image;
  // save to the database
  await user.save((err, updatedUser) => {
    const { _id, name, email, image, DOB } = updatedUser;
    res.status(200).json({
      _id,
      name,
      email,
